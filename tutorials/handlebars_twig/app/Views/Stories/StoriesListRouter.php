...
class StoriesListRouter extends AbstractAPI {
    ...
    protected function set_routes(){
        ...
        $this->dispatcher = \FastRoute\simpleDispatcher(function(\FastRoute\RouteCollector $r) {
            ...
            $BASE = '';
            $SET = '{set_num:\d+}';
            $ACTION = '{action:[stories|featured\-stories]+}';
            ...
            */
            ...
            $r->addRoute('GET', $BASE . '/' . $ACTION . '/',                          'get_action');  /*  /featured-stories/1/  */
            $r->addRoute('GET', $BASE . '/' . $ACTION,                                'get_action');  /*  /featured-stories/1  */

            $r->addRoute('GET', $BASE . '/' . $ACTION . '/' . $SET,                   'get_action');  // /featured-stories/1            
            $r->addRoute('GET', $BASE . '/' . $ACTION . '/' . $SET. '?{other:[^/]+}', 'get_action');  // /featured-stories/1? [anything but a slash]

            $r->addRoute('GET', $BASE . '/' . $ACTION . '/' . $SET . '/' . $FORMAT,                'get_action');  // /featured-stories/1/ajax
            $r->addRoute('GET', $BASE . '/' . $ACTION . '/' . $SET . '/' . $FORMAT . '/' . $DEBUG, 'get_action');  // /featured-stories/1/ajax/debug
